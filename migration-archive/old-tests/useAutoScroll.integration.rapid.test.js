...
describe('Rapid alternating pagination + appends integration', () => {
  test('pagination preserves read status, including across alternating prepend/append cycles', async () => {
    ...
    defineDims(el,{ scrollTop:600, scrollHeight:1000, clientHeight:400 }); // at bottom (distance 0)
    ...
    // User scrolls up significantly (to leave bottom zone)
    defineDims(el,{ scrollTop:400, scrollHeight:1025, clientHeight:400 }); 
    act(()=> el.dispatchEvent(new Event('scroll')));
    expect(hook.isAtBottom).toBe(false);
    
    // Append 2 more - should be marked as unread because not in read zone
    items = [...items, { id:'m31' }, { id:'m32' }]; 
    defineDims(el,{ scrollTop:400, scrollHeight:1075, clientHeight:400 });
    await act(async ()=> rerender(<Harness items={items} expose={h=>{ hook=h; }} />));
    expect(hook.newCount).toBe(2); // Outside read zone, so unread
    
    // Simulate pagination (prepend 10 older)
    const older = Array.from({ length:10 }).map((_,i)=>({ id:'o'+i }));
    items = [...older, ...items]; 
    defineDims(el,{ scrollTop:400, scrollHeight:1375, clientHeight:400 });
    await act(async ()=> rerender(<Harness items={items} expose={h=>{ hook=h; }} />));
    expect(hook.newCount).toBe(2); // Still 2 unread since pagination doesn't change unread count
