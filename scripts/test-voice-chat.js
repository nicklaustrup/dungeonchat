  ...
  try {
    ...
    recordTest(
      ...
      isSecure
        ...
        : "Not HTTPS - getUserMedia will fail"
    );
  } catch (error) {
...
async function runAllTests() {
  ...
  if (failedTests === 0) {
    console.log(
      chalk.green("✓ All tests passed! Voice chat should work perfectly.")
    );
  } else {
    console.log(
      chalk.yellow(
        `⚠ ${failedTests} test(s) failed. Review the failures above.`
      )
    );
    ...
    if (browserFailed > 0) {
      console.log(
        "• Browser: Consider adding polyfills or showing compatibility warnings"
      );
    }
    if (networkFailed > 0) {
      console.log(
        "• Network: Consider implementing adaptive quality or offline mode"
      );
    }
    if (securityFailed > 0) {
      console.log(
        "• Security: Review security implementations and add missing features"
      );
    }
