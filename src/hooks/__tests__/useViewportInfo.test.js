...
try { // eslint-disable-next-line global-require, import/no-extraneous-dependencies
	...
	useViewportInfo = function FallbackViewportHook(){ useEffect(()=>{},[]); };
}
...
describe('useViewportInfo', () => {
	...
	it('adds mobile class when width < 600', () => {
		...
		Object.defineProperty(global, 'innerWidth', {value: 500, configurable: true});
		const {unmount} = renderHook(() => useViewportInfo());
		act(() => { window.dispatchEvent(new Event('resize')); });
		expect(document.documentElement.classList.contains('mobile')).toBe(true);
		unmount();
		Object.defineProperty(global, 'innerWidth', {value: originalInnerWidth, configurable: true});
	});

	it('removes mobile class when width >= 600', () => {
		const originalInnerWidth = global.innerWidth;
		Object.defineProperty(global, 'innerWidth', {value: 800, configurable: true});
