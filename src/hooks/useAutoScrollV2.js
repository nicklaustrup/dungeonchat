 ...
 */
export function useAutoScrollV2({
  containerRef,
  anchorRef,
  items,
  threshold = 10,
  ...
  React.useEffect(() => {
    ...
    const isPagination =
      currentFirstId !== prevFirstId && currentLastId === prevLastId;

    // Detect new messages: last ID changed (newer messages added)
    const hasNewMessages =
      currentLastId !== prevLastId && currentLength > prevLength;
    ...
    if (hasNewMessages) {
      ...
      if (process.env.NODE_ENV === "development") {
        console.log("V2 hook: detected new messages", {
          newMessageCount,
          currentLastId,
          prevLastId,
        });
