 ...
 */
export function useUnifiedScrollManager({
  containerRef,
  anchorRef,
  messages,
  threshold = 10,
    ...
    (behavior = "smooth") => {
      ...
      if (anchor && behavior === "smooth") {
        ...
        setTimeout(() => {
          ...
          if (Math.abs(el.scrollTop - targetTop) > 5) {
            // If we're not close enough to bottom
    ...
    (classification) => {
      ...
      if (delta > 0) {
        ...
        const clampedTarget = Math.max(
          0,
          Math.min(targetScrollTop, el.scrollHeight - el.clientHeight)
        );
    ...
    if (isInitialLoadRef.current && currentMessages.length > 0) {
      ...
      const attemptScroll = (attempt = 0) => {
        ...
        setTimeout(
          ...
          50 + attempt * 100
        ); // Increasing delays: 50ms, 150ms, 250ms, 350ms
      };
