 ...
 */
export function useScrollPrependRestoration(
  containerRef,
  { ignoreBottomFrames = 2 } = {}
) {
  ...
  const markBeforeLoadMore = React.useCallback(
    (messages) => {
  ...
  const restoreIfNeeded = React.useCallback(
    (classification) => {
      ...
      if (process.env.NODE_ENV === "development") {
        console.log("Restoration hook classification:", {
          didPrepend,
          prependedCount,
          didAppend,
          reset,
      ...
      if (!didPrepend || didAppend || reset) {
        if (process.env.NODE_ENV === "development" && (didAppend || reset)) {
          console.log(
            "Restoration hook: skipping restore (append or reset detected)"
          );
        }
      ...
      const attemptAnchor = (phase) => {
        ...
        const desiredOffset =
          prev.anchorOffset != null ? prev.anchorOffset : currentOffset;
        ...
        applyScroll(
          phase === "immediate"
            ? "anchor-fallback"
            : "anchor-fallback-deferred",
          { offsetDelta, currentOffset, desiredOffset, phase }
        );
        return true;
      };
      ...
      requestAnimationFrame(() => {
        ...
        logEvent("restoration-skip", {
          delta,
          reason: "nonpositive-delta-no-anchor",
          prependedCount,
          deferredTried: true,
        });
      });
  ...
  const handleAfterMessages = React.useCallback(
    (messages) => {
