...
export function useViewportInfo() {
	...
	useEffect(() => {
		...
		mqCoarse.addEventListener ? mqCoarse.addEventListener('change', apply) : mqCoarse.addListener(apply);
		mqReduced.addEventListener ? mqReduced.addEventListener('change', apply) : mqReduced.addListener(apply);

		return () => {
			window.removeEventListener('resize', apply);
			window.removeEventListener('orientationchange', apply);
			mqCoarse.removeEventListener ? mqCoarse.removeEventListener('change', apply) : mqCoarse.removeListener(apply);
			mqReduced.removeEventListener ? mqReduced.removeEventListener('change', apply) : mqReduced.removeListener(apply);
		};
	}, []);
