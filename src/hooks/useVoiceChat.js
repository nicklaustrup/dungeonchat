...
export function useVoiceChat(
  ...
  const setupAudioLevelDetection = useCallback((userId, stream) => {
    try {
      ...
      console.error(
        `[useVoiceChat] Error setting up audio level detection:`,
        error
      );
    }
  }, []);
  ...
  useEffect(() => {
    ...
    managerRef.current.onReconnecting = (userId, attempt, maxAttempts) => {
      console.log(
        `[useVoiceChat] Reconnecting to ${userId} (attempt ${attempt}/${maxAttempts})`
      );
    };
  ...
  useEffect(() => {
    ...
    const joinedUsers = participants.filter(
      (p) => !previousParticipantIds.has(p.userId) && p.userId !== user?.uid
    );
    ...
    joinedUsers.forEach((participant) => {
      const displayName =
        participant.username || participant.displayName || "Someone";
      ...
      if (onNotificationRef.current) {
        onNotificationRef.current({
          ...
          message: participant.characterName
            ? `Playing as ${participant.characterName}`
            : null,
  ...
  const join = useCallback(async () => {
    ...
    try {
      ...
      await signalingRef.current.updatePresence(
        campaignId,
        roomId,
        user.uid,
        "online"
      ...
      signalingRef.current.listenForOffers(
        campaignId,
        roomId,
        user.uid,
        (fromUser, offer) => {
      ...
      signalingRef.current.listenForAnswers(
        campaignId,
        roomId,
        user.uid,
        (fromUser, answer) => {
      ...
      signalingRef.current.listenForIceCandidates(
        campaignId,
        roomId,
        user.uid,
        (fromUser, candidate) => {
      ...
      const existingParticipants = await voiceRoomService.getParticipants(
        firestore,
        campaignId,
        roomId
      );
