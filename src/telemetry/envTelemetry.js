...
export function getTelemetrySnapshot() { return [..._queue]; }
...
function computeEnv() {
  ...
  const orientation = (window.screen?.orientation?.type || (w > h ? 'landscape' : 'portrait')).split('-')[0];
  return {
...
function handleFirstInteraction(e) {
  ...
  logTelemetry({ type: 'first-interaction', sinceLoadMs: performance.now(), event: e.type });
...
function handleScroll() {
  ...
  requestAnimationFrame(() => {
    ...
    if (_scrollSamples.length >= 10) {
      const avg = Number((_scrollSamples.reduce((a,b)=>a+b,0)/_scrollSamples.length).toFixed(2));
