...
export function ChatStateProvider({ children, initialAwaySeconds = 300 }) {
  ...
  const actions = {
    ...
    setSearch: (searchTerm, showSearch) => dispatch({
      type: CHAT_ACTIONS.SET_SEARCH,
      payload: { searchTerm, showSearch }
    }),
    
    setReplyingTo: (replyTarget) => dispatch({
      type: CHAT_ACTIONS.SET_REPLYING_TO,
      payload: replyTarget
    }),
    
    setImageState: (imageData) => dispatch({
    ...
    setUploading: (uploading) => dispatch({
      type: CHAT_ACTIONS.SET_UPLOADING,
      payload: uploading
    }),
    
    setProfileModal: (user) => dispatch({
      type: CHAT_ACTIONS.SET_PROFILE_MODAL,
      payload: user
    }),
    
    setScrollMeta: (scrollMeta) => dispatch({
      type: CHAT_ACTIONS.SET_SCROLL_META,
      payload: scrollMeta
    }),
    
    setAwaySeconds: (seconds) => dispatch({
    ...
    removeImage: (imageId) => dispatch({
      type: CHAT_ACTIONS.REMOVE_IMAGE,
      payload: imageId
    }),
    
    clearAllImages: () => dispatch({
      type: CHAT_ACTIONS.CLEAR_ALL_IMAGES
    }),
    
    setImagesUploading: (uploading) => dispatch({
