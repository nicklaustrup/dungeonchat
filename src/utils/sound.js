...
export const playReceiveMessageSound = (enabled) => playAudio('notification', enabled);
export const playTapSound = (enabled) => playAudio('tap', enabled, { clone: true });
...
export function startTypingTapLoop(enabled, { minDelay = 100, maxDelay = 300, volume = 0.18 } = {}) {
  if (!enabled) return;
  preload();
