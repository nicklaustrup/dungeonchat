...
export async function fixTokenUserIds(firestore, campaignId) {
  ...
  try {
    ...
    for (const mapDoc of mapsSnapshot.docs) {
      ...
      for (const tokenDoc of tokensSnapshot.docs) {
        ...
        if (tokenData.characterId && !tokenData.userId) {
          ...
          const tokenRef = doc(firestore, 'campaigns', campaignId, 'vtt', mapId, 'tokens', tokenDoc.id);
      ...
      if (batchCount > 0) {
        await batch.commit();
        console.log(`✅ Committed final batch of ${batchCount} updates for map ${mapId}`);
      }
    ...
    console.log(`✅ Token userId migration complete! Fixed ${totalFixed} token(s) total.`);
    ...
    if (totalFixed > 0) {
      ...
      console.log('ℹ️ No tokens needed fixing. If HP sync still broken, check:');
      console.log('   1. Token has both characterId AND userId in Firestore');
      console.log('   2. Character document exists at campaigns/{campaignId}/characters/{userId}');
