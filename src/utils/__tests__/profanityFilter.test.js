...
describe("profanityFilter", () => {
  ...
  describe("filterProfanity", () => {
    test("filters profanity with first letter + asterisks", () => {
      expect(filterProfanity("This is fucking awesome")).toBe(
        "This is f****** awesome"
      );
      expect(filterProfanity("Holy shit that was great")).toBe(
    ...
    test("handles multiple profane words", () => {
      expect(filterProfanity("This shit is fucking terrible")).toBe(
        "This s*** is f****** terrible"
      );
    });

    test("preserves clean text unchanged", () => {
      expect(filterProfanity("This is a nice message")).toBe(
  ...
  describe("useProfanityFilter", () => {
    test("filters text when filter is enabled", () => {
      expect(useProfanityFilter("This is fucking awesome", true)).toBe(
        "This is f****** awesome"
      );
    });

    test("returns original text when filter is disabled", () => {
      expect(useProfanityFilter("This is fucking awesome", false)).toBe(
  ...
  describe("useProfanityFiltering hook", () => {
    test("returns filter functions with enabled state", () => {
      ...
      expect(result.current.filterText("This is fucking awesome")).toBe(
        "This is f****** awesome"
      );
      expect(result.current.containsProfanity("This is fucking awesome")).toBe(
        true
      );
    });
