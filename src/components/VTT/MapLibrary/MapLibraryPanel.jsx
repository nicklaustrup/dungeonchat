...
export default function MapLibraryPanel({
  firestore,
  campaignId,
  open,
  onClose,
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError(null);
      try {
        const all = await mapService.getMaps(firestore, campaignId);
        setMaps(all);
      } catch (e) {
        setError(e.message);
      } finally {
  ...
  const handleCreateFromUrl = async () => {
    ...
    try {
      ...
      const map = await mapService.createMap(firestore, campaignId, {
    ...
    <div className="map-library-panel">
      ...
      <div className="ml-body">
        <div className="ml-section">
          ...
          />
          <button onClick={handleCreateFromUrl} disabled={!url.trim()}>
            Create
          </button>
        </div>
        <div className="ml-section">
            ...
            {maps.map((m) => (
              <div key={m.id} className="ml-item" onClick={() => onSelect?.(m)}>
                ...
                <div className="ml-meta">
                  {m.gridSize}px grid • {m.visibility}
                </div>
              </div>
            ))}
            {maps.length === 0 && !loading && (
              <div className="ml-empty">No maps yet.</div>
            )}
          </div>
        </div>
      </div>
