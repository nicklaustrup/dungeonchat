...
import {
  diceService,
  parseInlineDiceCommand,
  formatRollForChat,
} from "../../services/diceService";
import { initiativeService } from "../../services/initiativeService";
import {
  getCharacterContext,
  getMessageContextType,
  cleanMessageText,
...
function ChatInput({
  ...
  const getPlayerName = React.useCallback(() => {
    ...
    if (campaignId && campaignMemberData) {
      ...
      if (campaignMemberData.role === "dm") {
        return (
          profile?.username || profile?.displayName || user.displayName || "DM"
        );
      }
  ...
  const activeImagePreview = hasContextImage
    ? contextPreview
    : imageHook.imagePreview;
  const activeImageFile = hasContextImage
    ? contextSelectedFile
    : imageHook.selectedImage;
  const isUploading = hasContextImage ? contextUploading : imageHook.uploading;

  const handleLocalFile = React.useCallback(
    (file) => {
  ...
  const handleSendBulkImages = React.useCallback(async () => {
    if (!bulkImages || bulkImages.length === 0 || bulkUploading || !user)
      return;
    ...
    try {
      ...
      pushToast("Some images failed to upload: " + err.message, {
        type: "error",
      });
  ...
  const { handleInputActivity } = useTypingPresence({
    rtdb,
    user,
    soundEnabled,
  });
    ...
    try {
      ...
      if (campaignId && trimmedText.startsWith("/init")) {
        try {
          ...
          await initiativeService.recordPlayerInitiativeRoll(
            firestore,
            campaignId,
            user.uid,
            total,
          ...
          await createTextMessage({
            ...
            diceData: {
              ...
              individual: [roll],
              breakdown:
                mod !== 0 ? `${roll}${mod > 0 ? `+${mod}` : mod}` : `${roll}`,
              notation: mod !== 0 ? `1d20${mod > 0 ? `+${mod}` : mod}` : "1d20",
              timestamp: Date.now(),
              initiative: true,
            },
          });
          ...
          pushToast("Failed to record initiative: " + error.message, {
            type: "error",
          });
          return;
        }
      ...
      if (diceCommand) {
        if (diceCommand.error) {
          pushToast(`Dice command error: ${diceCommand.error}`, {
            type: "error",
          });
          return;
        }
        ...
        try {
          ...
          const formattedRoll = formatRollForChat(
            rollResult,
            playerName,
            diceCommand.characterCommand
          );
        ...
        if (trimmedText.startsWith("/roll ") || trimmedText.startsWith("/r ")) {
          ...
          if (diceNotation) {
            try {
              ...
              pushToast(`Invalid dice notation: ${error.message}`, {
                type: "error",
              });
              return;
            }
            ...
            pushToast(
              "Please specify dice notation after /roll (e.g., /roll 1d20+5)",
              { type: "error" }
            );
            return;
          }
