...
export function BulkImagePreviewModal({ 
  ...
  const handleRemoveImage = React.useCallback((imageId, e) => {
    e.stopPropagation();
      ...
      <div className="bulk-image-preview" ref={dialogRef}>
        ...
        <div className="bulk-image-actions">
          ...
          >
            {uploading 
              ? 'Uploading…' 
              : (pendingSend ? 'Sending…' : `Send ${images.length} Image${images.length > 1 ? 's' : ''}`)
            }
          </button>
        ...
        <div className="bulk-image-hints" aria-hidden="true">
          <span>Esc to cancel · Ctrl/Cmd+Enter to send · Click trash icon to remove individual images</span>
        </div>
      </div>
    </div>
  );
