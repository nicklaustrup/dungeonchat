        ...
        <div className="result-main">
          ...
          {(isCrit || isFail) && (
            <div className="special-result">
              {isCrit && <span className="critical-hit">🔥 CRITICAL HIT!</span>}
              {isFail && (
                <span className="critical-fail">💀 CRITICAL FAIL!</span>
              )}
            </div>
          )}
        </div>
  ...
  if (recentRolls.length === 0) {
    return (
      <div className="dice-history-empty">
        ...
        <p>
          No dice rolls yet. Use the dice roller or type <code>/roll 1d20</code>{" "}
          in chat!
        </p>
      </div>
    );
        ...
        {recentRolls.map((roll, index) => (
          <div key={roll.id || index} className="history-item">
            {showChannelNames && roll.channelName && (
              <div className="channel-indicator">{roll.channelName}</div>
            )}
            <DiceRollDisplay
