...
function UserProfileModal({ user, userId, isOpen, onClose }) {
  ...
  const displayUser =
    user ||
    (profileData
      ? { ...profileData, id: targetUserId, uid: targetUserId }
      : null);
  if (!displayUser && !loading) {
    return null;
  }

  const fallbackAvatar = displayUser
    ? getFallbackAvatar({
        uid: displayUser.id || displayUser.uid,
        displayName: displayUser.username || displayUser.displayName,
        size: 128,
      })
    : "";
  const avatarSrc =
    displayUser?.profilePictureURL || displayUser?.photoURL || fallbackAvatar;
  ...
  const displayName =
    displayUser?.username || displayUser?.displayName || "Anonymous User";
        ...
        {!loading && !isOwnProfile && (
          <div className="profile-top-right-actions">
            {friendshipStatus === "none" && (
              <button
                className="profile-action-btn add-friend-top"
            ...
            {friendshipStatus === "pending_received" && (
              <span className="profile-action-note-top">Request received</span>
            )}
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="profile-body">
              <p>
                <strong>Status:</strong> {presenceLabel}
              </p>
              <p>
              ...
              <p style={{ fontSize: "12px", opacity: 0.7 }}>
                Away after {Math.round(awayAfterSeconds / 60)} min of
                inactivity.
              </p>
            </div>
            ...
            {!isOwnProfile && (
              <div className="profile-bottom-actions">
                {friendshipStatus === "blocked" ? (
                  <button
                    className="profile-action-btn unblock-bottom"
                  ...
                  <button
                    className="profile-action-btn block-bottom"
                    onClick={handleBlock}
                  >
                    Block User
                  </button>
