...
export function CharacterSheet({
  ...
  const { character, loading, error, updateCharacter, invalidateCache } =
  ...
  const handleHitPointChange = async (newHP) => {
    ...
    try {
      ...
      console.error(
        "❌ CharacterSheet.handleHitPointChange: Error updating hit points:",
        err
      );
  ...
  const handleCreateToken = async () => {
    ...
    try {
      ...
      alert(
        `Token created successfully for ${character.name}! Check the Token Manager to place it on the map.`
      );
    } catch (err) {
  ...
  const getAbilityTooltip = (ability) => {
    const tooltips = {
      strength:
        "Strength - Measures bodily power, athletic training, and physical force you can exert",
      ...
      intelligence:
        "Intelligence - Measures reasoning ability, memory, and analytical thinking",
      wisdom: "Wisdom - Measures awareness, intuition, and insight",
      charisma:
        "Charisma - Measures force of personality, leadership, and charm",
    };
  ...
  const renderCharacterHeader = () => {
    ...
    const displayImage =
      character.avatarUrl || character.photoURL || character.portraitUrl;
    ...
    return (
      <div className="character-header">
        ...
        <div className="character-title">
          ...
          <div
            className="character-subtitle"
            title="Character level, race, and class combination"
          >
        ...
        <div className="character-details">
          <div className="detail-item">
            ...
            <span title="Your character's life before becoming an adventurer, providing skills and personality traits">
              {character.background || "None"}
            </span>
          </div>
          <div className="detail-item">
            <label>Alignment</label>
            <span title="Your character's moral and ethical outlook (e.g., Lawful Good, Chaotic Neutral)">
  ...
  const renderAbilityScores = () => (
    <div className="ability-scores-section">
      <h3 title="The six core attributes that define your character's natural capabilities">
        Ability Scores
      </h3>
          ...
          ([ability, score]) => (
            ...
            >
              ...
              <div
                ...
                title="Modifier used for dice rolls and calculations"
              >
                {calculateAbilityModifier(score) >= 0 ? "+" : ""}
    ...
    <div className="combat-stats-section">
      ...
      <div className="combat-stats-grid">
        <div
          ...
          title="How difficult you are to hit in combat (10 + Dex modifier + armor bonus)"
        >
        ...
        <div
          className="stat-block"
          title="Bonus added to initiative rolls to determine turn order in combat"
        >
        ...
        <div
          className="stat-block"
          title="How far your character can move in a single turn (measured in feet)"
        >
          <label>Speed</label>
      ...
      <div className="hit-points-section">
        <h4 title="The amount of damage your character can take before falling unconscious">
          Hit Points
        </h4>
        <div className="hit-points-display">
          <div
            className="hp-current"
            title="Current hit points - decreases when you take damage"
          >
            <label>
              Current HP{" "}
              {pendingHP !== null && <span className="hp-pending">*</span>}
            </label>
            <div className="hp-input-wrapper">
              ...
              <input
                ...
                onChange={(e) =>
                  handleHPInputChange(parseInt(e.target.value) || 0)
                }
                min="0"
          ...
          <div
            className="hp-maximum"
            title="Maximum hit points - your character's full health"
          >
            <label>Max HP</label>
          ...
          {(character.temporaryHitPoints || 0) > 0 && (
            <div
              className="hp-temp"
              title="Temporary hit points - extra protection that's lost first"
            >
              <label>Temp HP</label>
  ...
  const renderSkills = () => (
    <div className="skills-section">
      <h3 title="Trained abilities that represent your character's expertise in various areas">
        Skills
      </h3>
      <div className="skills-grid">
        {Object.values(CHARACTER_SKILLS).map((skill) => {
          const modifier = calculateSkillModifier(character, skill.name);
          const isProficient = (character.skillProficiencies || []).includes(
            skill.name
          );
          ...
          return (
            <div
              ...
              title={`${skill.name} (${skill.ability.charAt(0).toUpperCase() + skill.ability.slice(1)}) - Represents your training and natural talent in this area`}
            >
  ...
  const renderSavingThrows = () => (
    <div className="saving-throws-section">
      <h3 title="Your character's ability to resist harmful effects and magical spells">
        Saving Throws
      </h3>
        ...
        {Object.entries(character.abilityScores || {}).map(
          ([ability, score]) => {
            ...
            const finalModifier = isProficient
              ? modifier + (character.proficiencyBonus || 0)
              : modifier;

            return (
              <div
                key={ability}
                className={`saving-throw-item ${isProficient ? "proficient" : ""}`}
                title={`${ability.charAt(0).toUpperCase() + ability.slice(1)} saving throw - Roll this when resisting effects that target this ability`}
              >
  ...
  const renderExperience = () => {
    ...
    return (
      <div className="experience-section">
        <h3 title="Experience points measure your character's professional development and combat expertise">
          Experience
        </h3>
        <div className="xp-display">
          ...
          <div
            className="xp-next"
            title="Experience points needed to reach the next level"
          >
    ...
    <div className="other-stats-section">
      ...
      <div className="other-stats-grid">
        <div className="stat-item">
          <label title="Bonus added to all ability checks, attack rolls, and saving throws when proficient">
            Proficiency Bonus
          </label>
          <span>+{character.proficiencyBonus || 2}</span>
        </div>
        <div className="stat-item">
          <label title="Your character's passive Perception score (10 + Perception modifier)">
            Passive Perception
          </label>
