...
export default function SessionQuickNav({
  activeTab,
  onNavigate,
  campaignId,
  firestore,
  ...
  useEffect(() => {
    ...
    const unsub = initiativeService.subscribeToInitiative(
      fs,
      campaignId,
      (data) => {
        setInitiative(data);
  ...
  useEffect(() => {
    ...
    const unsub = subscribeToEncounters(fs, campaignId, (encounters) => {
      const active = encounters.find(
        (e) => e.isActive && !e.isTemplate && !e.completedAt
      );
      ...
      {tabs.map((t) => {
        ...
        return (
          ...
          >
            ...
            {showEncounterBadge && (
              <span className="badge encounter">
                {activeEncounter?.name?.slice(0, 10) || "Enc"}
              </span>
            )}
            {showTurn && (
              <span className="badge turn">
                R{initiative.round} T{(initiative.currentTurn || 0) + 1}
              </span>
            )}
          </button>
        );
