    ...
    const fetchMemberData = async () => {
      try {
        const memberDoc = await getDoc(
          doc(firestore, "campaigns", campaignId, "members", userId)
        );
        if (memberDoc.exists()) {
...
function ChatMessage(props) {
  ...
  // Debug logging
  if (text && !safeText) {
    console.log("DEBUG: Text processing issue:", {
      originalText: text,
  ...
  if (safeText && !displayText) {
    console.log("DEBUG: Profanity filter issue:", {
      safeText,
  ...
  const { menuRef, menuPanelRef, menuMode, menuStyle, menuReady } =
    useMessageMenuPosition({
      menuOpen,
    ...
    if (campaignId && campaignMemberData) {
      ...
      if (campaignMemberData.role === "dm") {
        return (
          ...
          effectiveProfileData?.displayName ||
          displayName ||
          "Unknown DM"
        );
      }
  ...
  const fallbackAvatar = React.useMemo(
    () => getFallbackAvatar({ uid, displayName: userName, size: 40 }),
    [uid, userName]
  );
  ...
  const { reactions: reactionsState, toggleReaction } = useReactions({
    firestore,
    auth,
    messageId,
  ...
  const handleCopyText = async () => {
    if (text) {
      ...
      document.dispatchEvent(
        new CustomEvent("chat:prefill", { detail: { text } })
      );
    }
    setMenuOpen(false);
  };
  ...
  const renderCharacterContextIndicator = () => {
    if (!campaignId || !messageContext || messageContext === "neutral")
      return null;
    ...
    switch (messageContext) {
      case "in_character":
        return (
          <div
            className="character-context-indicator ic-indicator"
            title={
              characterName
                ? `${characterName} speaking in character`
                : "In character"
            }
          >
      ...
      case "out_of_character":
        return (
          <div
            className="character-context-indicator ooc-indicator"
  ...
  if (!showMeta) {
    ...
    return (
      <div
        ...
        onMouseEnter={() => onHoverMessage && onHoverMessage(messageId)}
        onMouseLeave={() => onHoverMessage && onHoverMessage(null)}
      >
        <div className="time-col" aria-hidden="true">
          {timeOnly}
        </div>
        <div className="message-content">
          {renderCharacterContextIndicator()}
          {deleted ? (
            <p className="deleted-message" aria-label="Message deleted">
            ...
            <>
              <div className="message-image">
                <img
                  ...
                  tabIndex={0}
                  onKeyDown={(e) => {
                    if (e.key === "Enter") setShowFullImage(true);
                  }}
                ...
                <p>
                  ...
                  {editedAt && (
                    <sub
                      ...
                      title={`Edited ${formatFullTimestamp(editedAt)}`}
                    >
                      {" "}
                      (edited)
                    </sub>
            ...
            (displayText || safeText) && (
              <p>
                {processMessageText(
                  displayText || safeText || "DEBUG: No text",
                  searchTerm
                )}{" "}
                {editedAt && (
                  <sub
  ...
  return (
      ...
      onClick={
        isTouch
          ? (e) => {
              ...
              if (["button", "img", "input", "textarea", "a"].includes(tag))
                return;
              handleSelect();
            }
          : undefined
    ...
    >
      <div className="message-inner">
        {replyTo && showMeta && (
          <div
            className="message-top-row"
            aria-label={`Replying to ${replyTo.displayName || "user"}`}
          >
              ...
              <InlineReplyContext
                ...
                onNavigate={(id) => {
                  ...
                  if (targetEl) {
                    ...
                    setTimeout(() => {
                      targetEl.classList.remove("reply-flash");
                    }, 600);
                    if (addedTemp)
                      setTimeout(() => {
