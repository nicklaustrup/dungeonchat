...
export const initiativeService = {
  ...
  removeCombatant: async (firestore, campaignId, combatantId) => {
    try {
      ...
      await runTransaction(firestore, async (tx) => {
        ...
        const updatedCombatants = (data.combatants || []).filter(
          (c) => c.id !== combatantId
        );
        let newCurrentTurn = data.currentTurn || 0;
        if (
          newCurrentTurn >= updatedCombatants.length &&
  ...
  startCombat: async (firestore, campaignId) => {
    try {
      ...
      const sortedCombatants = [...currentData.combatants].sort(
   ...
   */
  recordPlayerInitiativeRoll: async (
    firestore,
    campaignId,
    userId,
    rollTotal,
    ...
    try {
      ...
      await runTransaction(firestore, async (tx) => {
        ...
        const data = snap.data();
        let combatants = Array.isArray(data.combatants)
          ? [...data.combatants]
          : [];
        const existingIndex = combatants.findIndex(
          (c) => c.isPlayer && c.userId === userId
        );
        if (existingIndex >= 0) {
          combatants[existingIndex] = {
            ...combatants[existingIndex],
  ...
  addCharacterToInitiative: async (
    firestore,
    campaignId,
    character,
    userId
    ...
    try {
      const initiative =
        initiativeService.rollInitiativeForCharacter(character);
      ...
      return await initiativeService.addCombatant(
        firestore,
        campaignId,
        combatant
      );
  ...
  getCombatSummary: async (firestore, campaignId) => {
    try {
      const initiativeData = await initiativeService.getInitiativeData(
        firestore,
        campaignId
      );
      ...
      return {
        ...
        currentCombatant:
          initiativeData.combatants?.[initiativeData.currentTurn] || null,
      };
    } catch (error) {
      ...
      await runTransaction(firestore, async (tx) => {
        ...
        encounter.participants.forEach((p) => {
          ...
          for (let i = 0; i < qty; i++) {
            ...
            if (
              !existing.some((c) => c.id === baseId) &&
              !newCombatants.some((c) => c.id === baseId)
            ) {
              newCombatants.push({
                id: baseId,
                name:
