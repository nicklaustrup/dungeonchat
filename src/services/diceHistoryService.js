 ...
 */
export async function getCampaignDiceHistory(firestore, campaignId, limitCount = 50) {
  try {
    if (!campaignId) {
    ...
    for (const channelDoc of channelsSnapshot.docs) {
      try {
        ...
        console.warn(`Error fetching dice rolls from channel ${channelDoc.id}:`, channelError);
    ...
    if (error.code === 'failed-precondition' && error.message.includes('index')) {
      throw new Error('Dice history indexes are still building. Please wait a few minutes and try again.');
    }
 ...
 */
export async function getCampaignDiceStatistics(firestore, campaignId, userId = null) {
  ...
  const averageRoll = rollTotals.reduce((sum, total) => sum + total, 0) / totalRolls;
  const highestRoll = Math.max(...rollTotals);
  const lowestRoll = Math.min(...rollTotals);
  
  // Count criticals
  const criticalHits = filteredRolls.filter(roll => 
    roll.diceData?.individual?.includes(20) && 
    roll.diceData?.notation?.includes('d20')
  ).length;
  
  const criticalFails = filteredRolls.filter(roll => 
  ...
  // Find most used die
  const mostUsedDie = Object.entries(rollsByType)
    .sort(([,a], [,b]) => b - a)[0]?.[0] || null;
  
  // Recent activity (last 7 days)
  const weekAgo = new Date();
  weekAgo.setDate(weekAgo.getDate() - 7);
  const recentActivity = filteredRolls.filter(roll => 
    roll.timestamp > weekAgo
  ).length;
