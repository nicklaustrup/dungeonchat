...
export function isCriticalHit(rollResult) {
  if (
    !rollResult ||
    rollResult.error ||
    !rollResult.individual ||
    !Array.isArray(rollResult.individual)
  ) {
    return false;
  }
  return (
 ...
 */
export function formatRollForChat(
  rollResult,
  playerName,
...
export function parseInlineDiceCommand(message, character = null) {
  ...
  if (character) {
    ...
    if (skillCheck) {
      const skillModifier = getCharacterSkillModifier(
        character,
        skillCheck.skill
      );
      const totalModifier = skillModifier + (skillCheck.bonus || 0);
      const notation =
        totalModifier >= 0 ? `1d20+${totalModifier}` : `1d20${totalModifier}`;
    ...
    if (savingThrow) {
      const saveModifier = getCharacterSavingThrowModifier(
        character,
        savingThrow.ability
      );
    ...
    if (attackRoll) {
      ...
      const totalModifier =
        proficiencyBonus + abilityMod + (attackRoll.bonus || 0);
      const notation =
        totalModifier >= 0 ? `1d20+${totalModifier}` : `1d20${totalModifier}`;
...
export const COMMON_DICE_PRESETS = [
  ...
  { label: "2D6", notation: "2d6", description: "Two six-sided dice" },
  {
    label: "3D6",
    notation: "3d6",
    description: "Three six-sided dice (stats)",
  },
...
export const DND_ROLL_PRESETS = [
  ...
  },
  {
    label: "Advantage",
    notation: "2d20",
    description: "Roll with advantage (take higher)",
  },
  {
    label: "Saving Throw",
    notation: "1d20",
    description: "Basic saving throw",
  },
  {
    label: "Ability Check",
    notation: "1d20",
    description: "Basic ability check",
  },
  { label: "Initiative", notation: "1d20", description: "Initiative roll" },
  {
    label: "Hit Die (d6)",
    notation: "1d6",
    description: "Hit die for healing",
  },
  {
    label: "Hit Die (d8)",
    notation: "1d8",
...
export const SKILL_CHECK_PRESETS = [
  {
    label: "Perception",
    command: "/check perception",
    description: "Notice things in your environment",
  },
  {
    label: "Investigation",
    command: "/check investigation",
    description: "Search for clues and details",
  },
  {
    label: "Insight",
    command: "/check insight",
    description: "Read people and understand motives",
  },
  {
    label: "Stealth",
    command: "/check stealth",
    description: "Move unseen and unheard",
  },
  {
    label: "Athletics",
    command: "/check athletics",
    description: "Climb, jump, swim, or lift",
  },
  {
    label: "Acrobatics",
    command: "/check acrobatics",
    description: "Balance, tumble, or maneuver",
  },
  {
    label: "Persuasion",
    command: "/check persuasion",
    description: "Convince or negotiate",
  },
  {
    label: "Deception",
    command: "/check deception",
    description: "Lie or mislead",
  },
  {
    label: "Intimidation",
    command: "/check intimidation",
    description: "Threaten or coerce",
  },
  {
    label: "Arcana",
    command: "/check arcana",
    description: "Recall magical knowledge",
  },
  {
    label: "History",
    command: "/check history",
    description: "Recall historical facts",
  },
  {
    label: "Nature",
    command: "/check nature",
    description: "Recall natural knowledge",
  },
  {
    label: "Religion",
    command: "/check religion",
    description: "Recall religious knowledge",
  },
  {
    label: "Medicine",
    command: "/check medicine",
    description: "Heal or diagnose",
  },
  {
    label: "Survival",
    command: "/check survival",
    description: "Track, navigate, or forage",
  },
  {
    label: "Animal Handling",
    command: "/check animal handling",
    description: "Calm or control animals",
  },
  {
    label: "Performance",
  ...
  },
  {
    ...
    command: "/check sleight of hand",
    description: "Pick pockets or perform tricks",
  },
];
...
export const SAVING_THROW_PRESETS = [
  {
    label: "Strength Save",
    command: "/save strength",
    description: "Resist being moved or restrained",
  },
  {
    label: "Dexterity Save",
    command: "/save dexterity",
    description: "Dodge area effects",
  },
  {
    label: "Constitution Save",
    command: "/save constitution",
    description: "Resist poison, disease, or exhaustion",
  },
  {
    label: "Intelligence Save",
    command: "/save intelligence",
    description: "Resist mental effects",
  },
  {
    label: "Wisdom Save",
    command: "/save wisdom",
    description: "Resist charm, fear, or illusion",
  },
  {
    label: "Charisma Save",
    command: "/save charisma",
    description: "Maintain your sense of self",
  },
];
