...
describe('Bulk Image Upload Integration', () => {
  ...
  test('bulk file selection opens bulk upload modal', async () => {
    ...
    // Find the file input
    const fileInput = screen.getByLabelText(/upload image/i).parentElement.querySelector('input[type="file"]');
  ...
  test('removing individual images from bulk modal', async () => {
    ...
    expect(screen.getByRole('button', { name: /send 2 images/i })).toBeInTheDocument();
  });
