...
describe('EmojiMenu Error Handling', () => {
  ...
  it('should handle chunk loading errors in loadEmojiPicker function', async () => {
    ...
    global.import = jest.fn().mockRejectedValue(
      new Error('Loading chunk vendors-node_modules_emoji-picker-react_dist_emoji-picker-react_esm_js failed.')
    );

    // Import the EmojiMenu module after mocking
    const { EmojiMenuProvider } = await import('../../../components/ChatInput/EmojiMenu');
