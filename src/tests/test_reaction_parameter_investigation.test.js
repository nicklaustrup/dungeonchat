...
describe('Reaction Parameter Bug Investigation', () => {
  ...
  test('investigates desktop menu reaction parameter issue', async () => {
    ...
    const quickReactionBtns = screen.getAllByRole('button').filter(btn => 
      btn.className.includes('menu-reaction-btn')
    );
    
    const thumbsUpBtn = quickReactionBtns.find(btn => btn.textContent.includes('👍'));
    expect(thumbsUpBtn).toBeInTheDocument();
    
    // Log what happens when we click
    console.log('Before click - mockUpdateDoc calls:', mockUpdateDoc.mock.calls.length);
    fireEvent.click(thumbsUpBtn);
    
    // Wait a bit and check if anything was called
    await new Promise(resolve => setTimeout(resolve, 100));
    
    console.log('After click - mockUpdateDoc calls:', mockUpdateDoc.mock.calls.length);
