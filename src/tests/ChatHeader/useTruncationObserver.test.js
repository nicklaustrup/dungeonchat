...
describe("useTruncationObserver", () => {
    ...
    return (
      <div style={{ width }} data-testid="wrapper">
        <span
          ref={(el) => {
            register(el);
            if (el && expose) expose(el, recompute);
          }}
  ...
  test("sets data-truncated when content overflows", () => {
    ...
    Object.defineProperty(currentEl, "scrollWidth", {
      configurable: true,
      value: 300,
    });
    Object.defineProperty(currentEl, "clientWidth", {
      configurable: true,
      value: 100,
    });
    act(() => {
    ...
    const el2 = screen.getByTestId("observed");
