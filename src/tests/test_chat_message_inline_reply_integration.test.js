...
jest.mock('../utils/avatar', () => ({ getFallbackAvatar: () => 'data:image/png;base64,fallback2' }));
...
describe('ChatMessage InlineReplyContext integration', () => {
  test('renders inline reply context when replyTo provided', () => {
    const message = {
      ...
      replyTo: { id: 'orig1', uid: 'orig', displayName: 'Orig User', text: 'Original text' }
    };
  ...
  test('clicking snippet triggers scroll logic (no error)', () => {
    const message = {
      ...
      createdAt: new Date(),
      displayName: 'Child User',
      replyTo: { id: 'orig2', uid: 'orig', displayName: 'Orig User', text: 'Original B' }
    };
    render(<ChatMessage message={message} showMeta={true} />);
