import { initTelemetry, _resetTelemetryForTests, getTelemetrySnapshot } from '../../telemetry/envTelemetry';
...
describe('envTelemetry', () => {
  ...
  test('first interaction event logged once', () => {
    ...
    window.dispatchEvent(new PointerEvent('pointerdown'));
    const snap = getTelemetrySnapshot().filter(e => e.type === 'first-interaction');
