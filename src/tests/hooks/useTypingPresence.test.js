...
describe('useTypingPresence', () => {
  ...
  test('handleInputActivity triggers presence & typing state', () => {
    ...
    act(() => { result.current.handleInputActivity(1); });
    expect(setTyping).toHaveBeenCalledWith(base.rtdb, { uid: 'u1', displayName: 'Alice', typing: true });
  ...
  test('stops typing after inactivity', () => {
    jest.useFakeTimers();
    const { result } = renderHook(() => useTypingPresence(base));
    act(() => { result.current.handleInputActivity(2); });
    expect(setTyping).toHaveBeenCalledWith(base.rtdb, { uid: 'u1', displayName: 'Alice', typing: true });
    act(() => { jest.advanceTimersByTime(6000); });
    expect(setTyping).toHaveBeenCalledWith(base.rtdb, { uid: 'u1', displayName: 'Alice', typing: false });
    jest.useRealTimers();
  });

  test('no calls without user', () => {
    const { result } = renderHook(() => useTypingPresence({ ...base, user: null }));
    act(() => { result.current.handleInputActivity(5); });
