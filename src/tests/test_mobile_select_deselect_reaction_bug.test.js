...
describe("Mobile Selection/Deselection Reaction Bug", () => {
  ...
  beforeEach(() => {
    ...
    window.matchMedia = jest.fn().mockImplementation((query) => ({
      matches:
        query.includes("hover: none") && query.includes("pointer: coarse"), // Simulate touch device
      media: query,
      onchange: null,
      addListener: jest.fn(),
  ...
  afterEach(() => {
    console.error = console.error.mockImplementation
      ? jest.fn()
      : console.error;
  });
  ...
  test("MOBILE BUG: Rapid select/deselect cycles with reaction attempts", async () => {
    ...
    for (let cycle = 1; cycle <= 5; cycle++) {
      ...
      if (thumbsBtn) {
        ...
        console.log(
          `Cycle ${cycle}: Button found, classes:`,
          thumbsBtn.parentElement?.className
        );
        console.log(`Cycle ${cycle}: Button disabled:`, thumbsBtn.disabled);
        console.log(
          `Cycle ${cycle}: Button aria-hidden:`,
          thumbsBtn.parentElement?.getAttribute("aria-hidden")
        );
        ...
        console.log(
          `Cycle ${cycle}: Firebase calls after reaction attempt:`,
          mockUpdateDoc.mock.calls.length
        );

        if (consoleErrors.length > 0) {
          console.log(
            `Cycle ${cycle}: Console errors detected:`,
            consoleErrors
          );
        }
      } else {
        console.log(
          `Cycle ${cycle}: ❌ No reaction button found - this might be the issue!`
        );
      }
      ...
      console.log(
        `Cycle ${cycle}: Reaction items after cycle:`,
        currentReactionItems.length
      );

      // Look for any "no reaction" error patterns
      const hasErrorIndicators = consoleErrors.some(
        (error) =>
      ...
      if (hasErrorIndicators) {
        console.log(
          `🚨 Cycle ${cycle}: Potential "no reaction error" detected!`
        );
    ...
    console.log(
      "Final reaction state on UI:",
      screen.queryAllByTestId("reaction-item").length
    );
    ...
    const hasNoReactionError = consoleErrors.some(
      (error) =>
        error.includes("no reaction") ||
    ...
    if (hasNoReactionError) {
      console.log(
        "🚨 CONFIRMED: \"no reaction error\" detected during select/deselect cycles"
      );
  ...
  test("MOBILE BUG: Race condition between selection state and reaction state", async () => {
    ...
    console.log(
      "Race condition test - Firebase calls:",
      mockUpdateDoc.mock.calls.length
    );
    ...
    const raceConditionError = consoleErrors.some(
      (error) =>
        error.includes("no reaction") ||
    ...
    if (raceConditionError) {
      console.log(
        "🚨 Race condition detected between selection and reaction state"
      );
    } else {
