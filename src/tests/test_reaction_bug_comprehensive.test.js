...
describe('Reaction Bug Comprehensive Tests', () => {
  ...
  test('adding first reaction shows correct count', async () => {
    ...
    const thumbsUpBtn = screen.getAllByTestId('reaction-btn').find(btn => btn.textContent === '👍');
  ...
  test('accessibility attributes are correct for reactions', () => {
    ...
    const reactionItems = screen.getAllByTestId('reaction-item');
    const thumbsUp = reactionItems.find(item => item.textContent.includes('👍'));
    ...
    expect(thumbsUp).toHaveAttribute('aria-label', expect.stringContaining('2 users'));
    expect(heart).toHaveAttribute('aria-label', expect.stringContaining('1 user'));
  });
});