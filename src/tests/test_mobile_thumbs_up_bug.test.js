...
describe('Mobile Thumbs Up Emoji Bug', () => {
  ...
  test('thumbs up emoji should work on mobile after multiple select/deselect cycles', async () => {
    ...
    const thumbsUpBtn = reactionBtns.find(btn => btn.textContent.includes('👍'));
    ...
    const thumbsUpBtn2 = reactionBtns2.find(btn => btn.textContent.includes('👍'));
    ...
    const thumbsUpBtn3 = reactionBtns3.find(btn => btn.textContent.includes('👍'));
    ...
    console.log('✅ Thumbs up emoji worked reliably across multiple select/deselect cycles');
  });
